...
var extensionURL = 'https://ei-sugaya.github.io/xcx-my-blocks/dist/myBlocks.mjs';
/**
...
var ExtensionBlocks = /*#__PURE__*/function () {
  ...
  function ExtensionBlocks(runtime) {
    ...
    this.runtime.on('PROJECT_STOP_ALL', function () {
      // 'PROJECT_STOP_ALL'：ストップボタンや実行ボタンを押したときにruntimeオブジェクトから発行されるイベント。
      _this.resetAudio(); // 'PROJECT_STOP_ALL'を受け取ってWeb Audioの初期化をする。
  ...
  _createClass(ExtensionBlocks, [{
    ...
    value: function resetAudio() {
      // Web Audioの初期化メソッド：AudioContextのリセットをする（その時に鳴っているすべての音が止まる）
      if (this.audioCtx) {
        this.audioCtx.close();
      }
    ...
    value: function getInfo() {
      ...
      return {
        ...
        blocks: [{
          opcode: 'playTone',
          // プロジェクト・ファイル内に保存される中間コードの表現。'do-it'
          blockType: blockType.COMMAND,
          // ブロックの種類。BlockType.COMMAND：計算を実行して結果を利用しない。BlockType.REPORTER
          ...
          text: formatMessage({
            ...
            // [引数名]：引数の指定。'do it [SCRIPT]'
            description: 'tone' // 'execute javascript for example'

          }),
          func: 'playTone',
          // このブロックで呼び出す関数の名前。ExtensionBlocksクラスのplayToneメソッド。 'doIt'
          arguments: {
            // このブロックの引数を定義。
            FREQ: {
